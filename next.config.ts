...
/** GoDaddy/cPanel shared hosting — static HTML export (no Node runtime). */
const isGodaddyStatic = process.env.GODADDY_STATIC === "1";
...
const nextConfig: NextConfig = {
  ...
  /**
   * Static export for Apache/cPanel:
   * - `trailingSlash` so /about/ maps to about/index.html (GoDaddy-friendly)
   * - `images.unoptimized` — no `/_next/image` optimizer on shared hosting
   * - redirects live in deploy/godaddy/.htaccess (unsupported with `output: "export"`)
   */
  ...(isGodaddyStatic
    ? {
