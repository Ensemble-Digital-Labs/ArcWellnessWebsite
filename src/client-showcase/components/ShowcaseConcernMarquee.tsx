...
/** Match main-site hero keyword ticker: serif labels on signature teal bar. */
const LABEL_CLASS =
  "shrink-0 font-serif text-base font-semibold uppercase tracking-[0.14em] text-white sm:text-lg md:text-xl [text-shadow:0_1px_2px_rgba(44,44,44,0.22),0_1px_8px_rgba(0,0,0,0.12)]";
...
export function ShowcaseConcernMarquee() {
  return (
    <section
      className="overflow-hidden border-y border-white/15 bg-arc-teal shadow-[0_10px_36px_rgba(44,44,44,0.08),0_2px_24px_var(--arc-teal-glow)]"
          ...
          <span key={dup} className="inline-flex shrink-0 items-center gap-10 sm:gap-12 md:gap-14">
            {showcaseConcernMarqueeKeywords.map((label) => (
              <span key={`${dup}-${label}`} className="inline-flex shrink-0 items-center gap-10 sm:gap-12 md:gap-14">
                ...
                <span
                  className="select-none font-sans text-base font-semibold text-white/55 sm:text-lg md:text-xl"
                  aria-hidden
                >
                  ·
                </span>
