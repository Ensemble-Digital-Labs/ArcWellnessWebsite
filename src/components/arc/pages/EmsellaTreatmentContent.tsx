...
type EmsellaTreatmentContentProps = {
...
function CreamPlate({
  ...
  return (
    ...
    >
      {tileMedia ? (
        <CreamPlateTiledMedia src={emsellaCreamPlate.src} />
      ) : stableMedia ? (
        <div className="absolute inset-x-0 top-0 h-[min(280dvh,160rem)] w-full">
          <Image
            src={emsellaCreamPlate.src}
            alt=""
            fill
            sizes="100vw"
            unoptimized
...
function DarkPlate({ maskBottom = true }: { maskBottom?: boolean }) {
  return (
    ...
    >
      <Image
        src={emsellaDifferent.backgroundSrc}
        alt=""
        fill
        sizes="100vw"
...
/** EmSella — EXION design template (content/assets swap). */
export function EmsellaTreatmentContent({
  treatment,
}: EmsellaTreatmentContentProps) {
  return (
    <>
      {/* ---------- Hero ---------- */}
      <div
      ...
      >
        ...
        <span className="sr-only">{emsellaHero.imageAlt}</span>
      </div>
        ...
        >
          <div className="grid grid-cols-2 gap-x-5 gap-y-10 sm:grid-cols-3 md:grid-cols-5 md:gap-x-6 lg:gap-x-8">
            ...
            >
              <div className="flex h-full flex-col items-center text-center">
                <EmblemIcon
                  src={emsellaHero.poweredByIconSrc}
                  plate
                  className="h-24 w-24 sm:h-28 sm:w-28 md:h-[7.25rem] md:w-[7.25rem]"
                  iconClassName={emsellaHero.poweredByIconClassName}
                />
                <h3 className="mt-2 max-w-[13rem] sm:mt-3">
                  <TitleEmphasis className="block text-[clamp(1.55rem,3.8vw,2rem)] leading-[0.92] tracking-tight text-arc-teal-ink [-webkit-text-stroke:0.055em_color-mix(in_srgb,currentColor_55%,transparent)] [text-shadow:0_1px_2px_rgba(255,255,255,0.45),0.02em_0_0_color-mix(in_srgb,currentColor_35%,transparent),-0.02em_0_0_color-mix(in_srgb,currentColor_35%,transparent)]">
                    {emsellaHero.poweredByEyebrow}
                  </TitleEmphasis>
                </h3>
                <p className="mt-1.5 max-w-[14rem] font-sans text-sm leading-relaxed text-arc-charcoal/70 sm:mt-2 sm:text-[0.9375rem]">
                  {emsellaHero.synergyLine}
                </p>
              </div>
            </ArcTextReveal>
          </div>
              ...
              <div>
                <ArcTextReveal variant="heading">
                  <h2 className="text-balance leading-[0.88] text-arc-teal-ink">
                    {emsellaMechanism.titleLines.map((line, i) => (
                      <span
                        key={line}
                        className={cn(
      ...
      {/* ---------- Strength that supports every day ---------- */}
      <div
        className={cn(
            ...
            <div className="mx-auto max-w-4xl text-center">
              <ArcTextReveal variant="heading">
                <h2 className="text-balance leading-[0.92]">
                  ...
                  >
                    {emsellaTreatments.title} {emsellaTreatments.titleEmphasis}
                  </span>
                </h2>
              </ArcTextReveal>
              ...
              <div className="mx-auto max-w-lg text-center lg:mx-0 lg:max-w-none lg:text-left">
                <ArcTextReveal variant="heading">
                  <h2 className="text-balance leading-[0.92]">
                    ...
                    >
                      {emsellaDifferent.title} {emsellaDifferent.titleEmphasis}
                    </span>
                  </h2>
                </ArcTextReveal>
                <ArcTextReveal variant="body" delayIndex={1}>
                  ...
                  >
                    {emsellaDifferent.intro}
                  </p>
                </ArcTextReveal>
              </div>

              {/* EmSella: who it's for — 5 cards; row-2 pair centered under 3-up */}
              <div className="grid gap-x-8 gap-y-12 sm:grid-cols-2 lg:grid-cols-6">
                {emsellaDifferent.cards.map((card, i) => (
                  <ArcTextReveal
                    key={card.title}
                    variant="body"
                    delayIndex={i + 1}
                    className={cn(
                      "lg:col-span-2",
                      // Center the two-item second row under the three-up first row
                      i === 3 && "lg:col-start-2",
                      // On 2-col, center the lone 5th card
                      i === emsellaDifferent.cards.length - 1 &&
          ...
          >
            <div className={cn("relative z-10 mx-auto", ARC_PAGE_RAIL_MAX)}>
              <div className="mx-auto max-w-2xl text-center">
                <ArcTextReveal variant="heading">
                  <h2 className="text-balance leading-[0.92]">
                    ...
                    >
                      {emsellaExperience.title}{" "}
                      {emsellaExperience.titleEmphasis}
                    </span>
                  </h2>
                </ArcTextReveal>
              </div>
              ...
              <ol className="mt-14 grid grid-cols-2 gap-x-6 gap-y-12 sm:grid-cols-3 lg:grid-cols-5 lg:gap-6">
                {emsellaExperience.steps.map((step, i) => (
                  <ArcTextReveal
                    as="li"
                    key={step.title}
                    variant="body"
                    ...
                    className={cn(
                      ...
                      i === emsellaExperience.steps.length - 1 &&
      ...
      <InvestCTASection
        ...
        supportingLine={emsellaClosing.supportingLine ?? homeInvestSupport}
        topSeam
